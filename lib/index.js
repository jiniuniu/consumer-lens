...
import { register as registerTkVideoInfo } from './tools/cl-tk-video-info.js'
 ...
 *
 *   skill   cl-tk-comments        cl-shop-reviews     cl-reddit-posts
 *   tool    cl_tk_comments        cl_shop_reviews     cl_reddit_search
 *   目录     data/cl-tk-comments/  data/cl-shop-reviews/
 *
 ...
 * 但 /cl-tk-comments 整条链照样跑得完。让取数能力去赌一个 UI 依赖不划算。
 *
 ...
 *
 * `invocation` 两个都开：模型能自己调用，用户也能在对话框敲 /cl-tk-comments。
...
const SKILLS = [
  {
    ...
    description:
      ...
      '清掉噪声，然后语义聚类「评论区到底在讨论什么」——不预设是痛点，' +
      '可能是羡慕/玩梗/质疑/跑题，每种指向不同用途。' +
      '触发词：/cl-tk-comments、扒这条视频的评论、这个视频评论区在说什么、评论聚类。',
...
export function apply(ctx, userConfig) {
  ...
  // ① 三个 tool —— 模型调
  registerTkVideoInfo(ctx, config)
  ...
  // ③ 面板路由 —— 全是 GET，面板从不写数据。
  //    写是 skill 的事（用户在对话框里跑 /cl-tk-comments），面板只读 + 发指令。
    ...
    sub.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          if (p.endsWith('/list')) {
            return send(res, 200, await store.list('cl-tk-comments'))
          }

          if (p.endsWith('/item')) {
            const id = url.searchParams.get('id') ?? ''
            const d = await store.read('cl-tk-comments', id)
