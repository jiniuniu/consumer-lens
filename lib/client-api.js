 ...
 */
export async function call(ctx, apiBase, path, body = null) {
  // 每次调用重新解析，不缓存 —— 用户换 key 不用重启 dsh
  const token = await resolveToken(ctx)
  if (!token) {
    throw new Error(
      'CL_TOKEN 未配置。请在 dsh 的 设置 → 插件 → Consumer Lens 里填入 token。',
    )
  }
...
async function resolveToken(ctx) {
  ...
  if (credentials !== undefined) {
    try {
      const v = await credentials.resolve(CL_TOKEN)
      if (v) return v
    } catch {
      /* 解析失败时退回环境变量 */
    }
  }
  return process.env[CL_TOKEN] ?? ''
}
